...
def number_of_ways_to_arrive_at_destination(n: int, roads: List[List[int]]) -> int:
    ...
    while q.qsize():
        time, isec = q.get()
        # have to update times
        for aisec, atime in adj[isec]:
            ntime = time + atime
            if ntime < times[aisec]:
                times[aisec] = ntime
                q.put((ntime, aisec))
                ways[aisec] = ways[isec]
            elif ntime == times[aisec]:
                ways[aisec] += ways[isec]

    # return modulo of 10**9 + 7 of ans
    return ways[n - 1] % (10**9 + 7)


if __name__ == '__main__':
    print(number_of_ways_to_arrive_at_destination())
